...
/**
 * Encode a mission ID for final quiz URLs
 * @param {string} id - The MongoDB ID of the mission
 * @returns {string} The encoded ID
 */
export function encodeFinalQuizId(id) {
  if (!id) return "";
  let encoded = btoa(id.toString());
  const prefix = "finalquiz_";
  const suffix = "_final" + Math.floor(Math.random() * 1000);
  encoded = encoded.replace(/=/g, "");
  encoded = encoded.replace(/\+/g, "-");
  encoded = encoded.replace(/\//g, "_");
  return `${prefix}${encoded}${suffix}`;
...
/**
 * Decode an encoded mission ID from final quiz URLs
 * @param {string} encodedId - The encoded ID
 * @returns {string} The original MongoDB ID
 */
export function decodeFinalQuizId(encodedId) {
  if (!encodedId) return "";
  try {
    // First, extract just the encoded part
    let encoded = encodedId;

    // Handle full slug format (e.g., "final-quiz-mission-finalquiz_abc123_final456")
    const finalquizIndex = encoded.indexOf("finalquiz_");
    if (finalquizIndex !== -1) {
      encoded = encoded.substring(finalquizIndex);
    }

    // Now we should have something like "finalquiz_abc123_final456"
    if (encoded.startsWith("finalquiz_")) {
      encoded = encoded.substring(10); // Length of "finalquiz_"
    }

    const finalIndex = encoded.indexOf("_final");
    if (finalIndex !== -1) {
      encoded = encoded.substring(0, finalIndex);
    }
    ...
    console.log("Prepared for decoding final quiz:", encoded);
    ...
    console.error(
      "Failed to decode final quiz ID:",
      error,
      "Original ID:",
      encodedId
    );
...
/**
 * Generate a slug for final quiz
 * @param {string} missionId - The MongoDB ID of the mission
 * @param {string} missionTitle - The mission title
 * @returns {string} The slug for the URL
 */
export function generateFinalQuizSlug(missionId, missionTitle = "") {
  const encodedId = encodeFinalQuizId(missionId);

  // If title is provided, create a SEO-friendly slug
  if (missionTitle) {
    const slug = missionTitle
      .toLowerCase()
      .replace(/[^\w\s-]/g, "") // Remove special characters except spaces and hyphens
      .replace(/\s+/g, "-") // Replace spaces with hyphens
      .substring(0, 50); // Limit length

    return `${slug}-final-quiz-${encodedId}`;
  }

  return `final-quiz-${encodedId}`;
}
