...
export default function InstructorDashboard() {
  ...
  const { setUserProfile, userProfile, setUserUpdatedValue } = useContext(InstructorContext);
  ...
  async function checkAuthAndFetchProfile() {
    try {
      ...
      // Make sure you have the profilebase import - you'll need to add this
      // import { profilebase } from 'your-api-config-path';
      
      const response = await profilebase.get(`/user-profile/${parsedUser._id}`, {
        headers: {
  ...
  return (
    <div className="flex flex-col h-full gap-10 w-full ">
      <div className="h-fit w-full">
        <div className="hidden sm:flex h-fit justify-between w-full">
          <div>
            ...
            <p className="text-[var(--text)] text-[12px] xl:text-[15px] leading-[40px] ">
              ...
              {userProfile && (
                <span className="ml-2 text-[#604196] font-semibold">
        ...
        <div className="h-fit grid gap-5 xl:grid-cols-2">
          <div className="grid grid-cols-2 gap-3 h-full">
            {data.map((items) => (
              <div key={items.id} className="py px bg-[#0F0F0F] gap-5 flex flex-col justify-center rounded-[17px]">
                <p className="text-[14px] leading-[150%] sm:text-[20px] text-[#C7C7C7] sm:leading-[40px] ">{items.sub}</p>
                <p className="font-[600] text-[41px] leaning-[150%] sm:text-[66px] text-[var(--background)] sm:leading-[40px] ">{items.qunatity}</p>
              </div>
            ))}
          </div>
            ...
            <div className="w-full flex items-center justify-between ">
              <div>
                <div className="relative py px bg-[#0F0F0F] gap-5 flex flex-col justify-between rounded-[17px]">
                  <p className="h-[30px] absolute top-[20px] right-[-50px] w-[56px] bg-[#2C2C2C] rounded-[6px] text-[#66CB9F] font-[700] text-[12px] leading-[16px] flexcenter ">+12%</p>
                  <p className="font-[600] text-[41px] leaning-[150%] sm:text-[66px] text-[var(--background)] sm:leading-[40px] ">25.500</p>
                  <p className="text-[14px] leading-[150%] sm:text-[20px] text-[#C7C7C7] sm:leading-[40px] ">Course Engagement</p>
                </div>
              </div>
      ...
      <div className="h-fit gap-5 w-full grid xl:grid-cols-5">
        <div style={{padding:'10px'}} className="xl:col-span-2 px py bg-[#0F0F0F] rounded-[20px] w-full h-full">
          <div style={{marginBottom:'25px'}} className="h-[40px] w-full flex items-center justify-between ">
            <p className="font-[700] text-[20px] leading-[40px] text-[var(--background)] ">Recent Activities</p>
          ...
          <div className="w-full h-[450px] flex-1 flex flex-col gap-10 overflow-auto scrollbar-hide">
            {studentcourse.slice(0,4).map((el,i) => (
              <div key={el.id} className="h-[112px] flex items-center gap-5 w-full rounded-[15px] bg-[#0F0F0F] ">
                ...
                <div>
                  <p className="font-[500] text-[18px] leading-[40px] ">{el.name}<span className="font-[400] text-[var(--text)] ">{el.purpose}</span></p>
                  <p className="text-[#774A82] text-[15px] leading-[40px] font-[500] ">{el.time}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{marginBottom:'25px'}} className="xl:col-span-2 px py w-full h-fit">
            ...
            <div className="w-full h-[450px] flex-1 flex flex-col gap-1 overflow-auto scrollbar-hide">
              <div className="grid grid-cols-5">
                <div className="col-span-2 text-[#7C7C7C] font-[600] text-[11px] sm:text-[13px] leading-[40px] ">Mission</div>
                <div className="col-span-1 text-[#7C7C7C] font-[600] text-[11px] sm:text-[13px] leading-[40px] ">Recruits</div>
                <div className="col-span-1 text-[#7C7C7C] font-[600] text-[11px] sm:text-[13px] leading-[40px] ">Revenue</div>
                <div className="col-span-1 text-[#7C7C7C] font-[600] text-[11px] sm:text-[13px] leading-[40px] ">Rating</div>
              </div>
                    ...
                    <div className="grid py h-fit rounded-[15px] bg-[#0F0F0F] grid-cols-5">
                      <div className="col-span-2 ">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-[700] text-[12px] leading-[20px] sm:text-[16px] sm:leadig-[20px] ">{el.purpose}</p>
                            <p className="font-[700] text-[12px] leading-[20px] sm:text-[16px] sm:leadig-[20px] ">{el.type}</p>
                          </div>
                        </div>
                      </div>
                      <div className="col-span-1 self-center text-[#7C7C7C] font-[600] text-[11px] sm:text-[13px] leading-[40px] ">{el.recruits}</div>
                      <div className="col-span-1 self-center text-[#7C7C7C] font-[600] text-[11px] sm:text-[13px] leading-[40px] ">{el.revenue}</div>
                      <div className="col-span-1 self-center w-fit ">
                        ...
                        <p className="text-[#7C7C7C] text-center font-[600] text-[13px] leading-[40px] ">{el.rating}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <div style={{marginBottom:'20px'}} className="h-fit w-full flex flex-col gap-3 ">
        <div style={{marginBottom:'25px'}} className="h-[40px] w-full flex items-center justify-between ">
          <p className="font-[700] text-[20px] leading-[40px] text-[var(--background)] ">Active Missions</p>
          <p className="text-[#208045] font-[500] text-[15px] leading-[40px] ">View All</p>
        </div>
