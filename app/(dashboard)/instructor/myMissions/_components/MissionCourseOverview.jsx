...
const LessonItem = ({ lesson }) => (
  <div className="flex items-center gap-3 border-[0.5px] border-[#535353] bg-[#352e2b] rounded-[8px]" style={{padding: '16px'}}>
...
const ChapterSection = ({ chapter }) => (
  <div className="flex flex-col gap-5 rounded-[15px] border border-[#535353]" style={{margin: '0 30px'}}>
    <div className="w-full border-b border-[#535353]" style={{padding: '16px'}}>
      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
        <div style={{paddingLeft: '48px'}}>
          <p className="text-[18px] font-[700] leading-[40px]">{chapter.title}</p>
...
const QuickActionCard = ({ action }) => (
  <div className="bg-[#0F0F0F] h-[145px] rounded-[12px] flex items-center gap-4" style={{padding: '20px'}}>
    <div className={`h-[72px] w-[72px] rounded-[10px] ${action.bgColor} ${action.textColor} flex items-center justify-center text-xl font-bold`}>
      {action.icon}
    </div>
    <div className="flex-1">
      <p className="text-[20px] font-[700] leading-[28px]" style={{marginBottom: '4px'}}>{action.title}</p>
...
const TabNavigation = ({ activeTab, setActiveTab }) => {
  const tabs = ['Curriculum', 'Students', 'Resources', 'Interactions', 'Settings'];
...
const CurriculumContent = ({ course }) => (
  <>
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" style={{padding: '0 32px'}}>
      <h2 className="text-[24px] font-[600]">Course Curriculum</h2>
      <button className="bg-[#5E36A5] hover:bg-[#4A2D85] transition-colors rounded-[8px] text-white font-medium" style={{padding: '8px 16px'}}>
        + Add Chapter
      </button>
    </div>
...
export default function MissionCourseOverview({ course }) {
  ...
  return (
    <div className="flex flex-col w-full max-w-[1200px] mx-auto gap-10" style={{padding: '16px'}}>
      {/* Course Header */}
      <div className="grid xl:grid-cols-3 gap-8">
        <div className="border border-[#6D4879] w-full h-[300px] xl:h-full rounded-[13px] bg-gradient-to-br from-[#2D1B3D] to-[#1A0F21] flex items-center justify-center">
          <div className="text-center text-[#6D4879]">
            <div className="text-4xl" style={{marginBottom: '8px'}}>📚</div>
        ...
        <div className="xl:col-span-2 flex flex-col gap-8">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <button className={`rounded-[4px] text-[10px] text-black font-[600] ${
              course.status === 'Published' ? 'bg-[#7BBD25]' : 'bg-[#7343B3]'
