...
export default function Authnav() {
  const { registerStep, setRegisterStep } = useContext(AcademyGloblaxcontex);
  ...
  return (
    <div className="relative  w-full sm:block hidden flexcenter">
      ...
      <div
        onClick={handleClick}
        className={`absolute h-[30px] top-[3%] hidden sm:block  w-[44px] sm:left-[5%] xl:left-[10%] cursor-pointer flex items-center justify-center
          ${registerStep === 1 || registerStep === 5  ? "opacity-40 cursor-not-allowed" : "opacity-100 hover:opacity-70"}`}
      >
        ...
        <div className="w-full   flex gap-5 items-center max-w-[350px] sm:max-w-[561px]">
          ...
          <div
            className={`h-[2px] w-full ${
              registerStep >= 2 ? "bg-[var(--secondary)]" : "bg-[var(--text)]"
            }`}
          ></div>
